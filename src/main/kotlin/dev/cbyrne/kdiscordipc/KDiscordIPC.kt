    ...
    suspend fun connect(index: Int = 0) {
        ...
        socketHandler.events.collect {
            when (it) {
                ...
                is DispatchEventPacket.GuildCreate -> _events.emit(GuildCreateEvent(it.data))
                is DispatchEventPacket.GuildStatus -> _events.emit(GuildStatusEvent(it.data))
                is DispatchEventPacket.MessageCreate -> _events.emit(MessageCreateEvent(it.data))
                is DispatchEventPacket.MessageUpdate -> _events.emit(MessageUpdateEvent(it.data))
                is DispatchEventPacket.MessageDelete -> _events.emit(MessageDeleteEvent(it.data))
                is DispatchEventPacket.Notification -> _events.emit(NotificationEvent())
